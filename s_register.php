<?php
        // Initialize error message
          ...
          if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $sql = "INSERT INTO suppliers (username, first_name, last_name, phone_number, password) VALUES (?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
        
            // Hash the password before storing it in the database (assuming password hashing is implemented)
            $hashedPassword = password_hash($password, PASSWORD_DEFAULT);
        
            if ($stmt->execute([$username, $firstName, $lastName, $phone, $hashedPassword])) {
              // Registration successful (redirect or confirmation message)
              $message = 'Registration successful! Please <a href="s_login.php">login</a> to continue.';
            } else {
...
<html>
    ...
    <body>
        <h1 style="text-align: center;">Supplier Register</h1>
        <?php if ($message): ?>
        ...
        <div>
            <div style="text-align: center;">Already registered? <a href=s_login.php>Login</a> now!</div>
        </div>
        <script>
